...
select max(margin), `year`, `won_by`
 FROM `ipl`.`match_summary` where `won_by`='runs' and winner='RCB' group by `year`;
...
SELECT batsmanName,sum(runs) as total_runs FROM batting_summary inner join `ipl`.`match_summary` 
on batting_summary.match_id=`ipl`.`match_summary`.match_id
...
SELECT bowlerName,sum(wickets) as total_wickets FROM bowling_summary inner join `ipl`.`match_summary` 
on bowling_summary.match_id=`ipl`.`match_summary`.match_id
...
SELECT winner,count(`winner`) as total_wins FROM `ipl`.`match_summary` group by winner order by total_wins desc;

# Chasing Time winner
SELECT `team2`,COUNT(*) as Won_by_chase
FROM `ipl`.`match_summary`
where team2=winner
group by team2
order by Won_by_chase desc;

#target defend
SELECT `team1`,COUNT(*) as target_defend
FROM `ipl`.`match_summary`
where team1=winner
group by team1
...
#won bY
SELECT `won_by`,count(won_by) AS won_by_count FROM `ipl`.`match_summary` where  year =2021 group by won_by;
